...
vori = @model vori begin
    ...
    @parameters begin
        ...
        # VenIC = 0.0, [unit = u"mg", description = "Venous blood initial drug amount"]
    end
    ...
    @variables begin
        ...
        (Ven(t) = 0.0), [unit = u"mg", description = "Venous drug"]
...
getUnit(vori.parameters.Vad)
getDescription(vori.parameters.Vad)
getDefault(vori.parameters.Vad)
vori.parameters.Vad = 99
vori.parameters.Vad
getDefault(vori.parameters.Vad)
getDefaultExpr(vori.parameters.CL_Ki)


ev1 = PMEvents.MMInput(time = 1.0, amt = 4.0, tinf = 0.5, input = :venousInfusion);
ev2 = PMEvents.MMInput(time = 10.0, amt = 0.25, input = :Ven);
evs = PMEvents.collect_evs([ev1,ev2], vori); # Working on moving this into the solve function, should just be able to pass a vector of events

vori._odeproblem.tspan = (0.0, 25.0) # This is kind of hidden, working on a fix...
vori_orig = deepcopy(vori); # Grab a copy for later!


# vori.parameters.Vad = getDefault(vori.parameters.Vad)
...
ParameterizedModels.solve!(vori_orig, callback=evs);
plot(vori_orig._solution.t, vori_orig._solution.Ven)
